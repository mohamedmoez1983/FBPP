...
H2Oviscosity = lambda T: 2.414E-5 * ureg('kg/m/s') * 10 ** (
            247.8 * ureg.K / (T - 140 * ureg.K))  # (Pa.s)=(kg/(m.s)) accurate to within 2.5% from 0 °C to 370 °C
...
def gelplot_imshow(distances, bandwidths, intensities, lanes, names,
                   ...
                   FWTM, show=True):
    parent = tk.Tk()
    parent.geometry('675x690+250+25')
    nlanes = len(lanes)
...
#       + type(_np.array(2)*_np.array(2)/_#!/usr/bin/env python3
# # -*- coding: utf-8 -*-
# # doctest: +NORMALIZE_WHITESPACE
# # doctest: +SKIP
#
